...
x_reform = reshape(x_flat,[256,256]);
...
A_flat = reshape(A,65536,20); % Let's flatten out our basis images

% Project x onto A using equation (A'*A)^(-1)*A'*x'
    % fun fact: you have to break down the operation, because computing this
    % all on one line exceeds the 32GB array size. 

tmp = (A_flat'*A_flat);
tmp = tmp^-1;
tmp = tmp*A_flat'*x_flat;
x_hat = A_flat*tmp;

% The error is the difference between x and x hat
e = x_flat - x_hat;
% The following normalizes the image so the max intensity is 1. Otherwise, most
% pixels will be too dim to even see. Think of this as turning up the
% brightness
e_scale = e/max(e); 
e_scale = reshape(e_scale,256,256);
figure;
imshow(e_scale);
