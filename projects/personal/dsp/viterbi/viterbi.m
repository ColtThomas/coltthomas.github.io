function [decoder_output,survivor_state,cumulated_metric]=viterbi(G,k,channel_output)
    %VITERBI
    % the viterbi decoder for conolutional codes [decoder_output,survivor_state,cumnulated_metric]=viterbi(G,k,channel_output)
    % G is a n x Lk matrix each row of which determines the connections
    % from the shift register to the n-th outptu of the ocde, k/n is the
    % rate of the code. survivor_state is a matrix showing the optimal path
    % through the trellis. The metric is given in a separate function
    ...
    %check the sizes
    if rem(size(G,2),k) ~=0
        error('size of g and k dont agree')
    end
   ...
   state_sequence(1,depth_of_trellis+1)=survivor_state(1,depth_of_trellis+1);
...
% source: Contemporary Communication Systems using Matlab chapter 8 pg 382-384
