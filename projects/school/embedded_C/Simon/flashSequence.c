...
#define FLASHSEQ_TIMEWAIT 20								// Time that each square will display
...
void flashSequence_tick() {
	...
	case start_seq_st:
		flashState = wait_st;
		simonDisplay_drawSquare(globals_getSequenceValue(arrayAccess),false);
		break;
	case wait_st:

		if (wait >= FLASHSEQ_TIMEWAIT && inc != globals_getSequenceIterationLength()) {
			simonDisplay_drawSquare(globals_getSequenceValue(arrayAccess),true);
			inc++;
			arrayAccess++;
			flashState = start_seq_st;
		}
		else if (wait >= FLASHSEQ_TIMEWAIT && inc == (globals_getSequenceIterationLength())){
			simonDisplay_drawSquare(globals_getSequenceValue(arrayAccess),true);
			flashState = final_st;
		}
		else {
