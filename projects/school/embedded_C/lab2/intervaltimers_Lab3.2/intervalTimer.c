...
// All address locations used for the timer
...
uint32_t intervalTimer_getTotalDurationInSeconds(uint32_t timerNumber, double *seconds) {
	...
	//figure out how to access the timer values and save them to *seconds
	switch (timerNumber) {

	case 0:
			returnTime = (readRegister(XPAR_AXI_TIMER_0_BASEADDR + INTERVALTIMER_TCR0_ADDRESS) / (double) XPAR_AXI_TIMER_0_CLOCK_FREQ_HZ) + (readRegister(XPAR_AXI_TIMER_0_BASEADDR + INTERVALTIMER_TCR1_ADDRESS) *  INTERVALTIMER_TCR1_MOD);
			*seconds =  returnTime ;
			return INTERVALTIMER_VALID_TIMER_TRUE;
		break;
	case 1:
			returnTime = (readRegister(XPAR_AXI_TIMER_1_BASEADDR + INTERVALTIMER_TCR0_ADDRESS) / (double) XPAR_AXI_TIMER_1_CLOCK_FREQ_HZ) + (readRegister(XPAR_AXI_TIMER_1_BASEADDR + INTERVALTIMER_TCR1_ADDRESS) * INTERVALTIMER_TCR1_MOD);
			*seconds =  returnTime ;
			return INTERVALTIMER_VALID_TIMER_TRUE;
		break;
	case 2:
			returnTime = (readRegister(XPAR_AXI_TIMER_2_BASEADDR + INTERVALTIMER_TCR0_ADDRESS) / (double) XPAR_AXI_TIMER_1_CLOCK_FREQ_HZ) + (readRegister(XPAR_AXI_TIMER_2_BASEADDR + INTERVALTIMER_TCR1_ADDRESS) * INTERVALTIMER_TCR1_MOD);
			*seconds =  returnTime ;
			return INTERVALTIMER_VALID_TIMER_TRUE;
		break;
	default:
